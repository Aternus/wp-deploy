...
- [ ] Project root contains a valid project manifest (`package.json`,
      `pyproject.toml`, `pom.xml`, `build.gradle`, `go.mod`, `*.csproj`,
      `Gemfile`, or `Cargo.toml`)
- [ ] No existing test framework detected that conflicts with the target setup
- [ ] Project type identifiable (React, Vue, Angular, Next.js, Node, Python,
      Java, Go, .NET, Ruby, Rust, etc.)
...
- [ ] Project manifest successfully read and parsed (`package.json`,
      `pyproject.toml`, `pom.xml`, `go.mod`, etc.)
...
- [ ] Framework choice justified (Playwright vs Cypress for frontend;
      pytest/JUnit/Go test/xUnit/RSpec for backend)
- [ ] Framework preference respected (if explicitly set via
...
- [ ] Minimal test script added to appropriate config (`package.json` for
      frontend, `Makefile`/`pyproject.toml`/`build.gradle` for backend)
...
- [ ] Shell scripts use `PACTICIPANT` env var pattern (not hardcoded service
      names)
...
- [ ] Consumer tests use PactV4 `addInteraction()` builder (not PactV3 fluent
      API)
...
- [ ] Consumer code exposes URL injection mechanism (`setApiUrl()`, env var, or
      constructor param)
- [ ] Local consumer-helpers shim present if `@seontechnologies/pactjs-utils`
      not installed
...
- [ ] User can run the appropriate test command without errors
      (`npm run test:e2e`, `pytest`, `go test ./...`, etc.)
...
**Completed by:** {name} **Date:** {date} **Framework:** { Playwright / Cypress
or something else} **Notes:** {notes}
