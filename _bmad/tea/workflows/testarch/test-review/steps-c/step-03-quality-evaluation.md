...
description:
  'Orchestrate adaptive quality dimension checks (agent-team, subagent, or
  sequential)'
...
Select execution mode deterministically, then evaluate quality dimensions using
agent-team, subagent, or sequential execution while preserving output contracts:
...
In `agent-team` and `subagent` modes, runtime decides worker scheduling and
concurrency.
...
Pass the same `timestamp` value to Step 3F (do not regenerate it). Step 3F must
read the exact temp files written in this step.
