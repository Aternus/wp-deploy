...
Read outputs from 4 quality subagents, calculate weighted overall score (0-100),
and aggregate violations for report generation.
...
- **If `{outputFile}` already exists**, update:
  - Add `'step-03f-aggregate-scores'` to `stepsCompleted` array (only if not
    already present)
...
**Master Rule:** Aggregate determinism, isolation, maintainability, and
performance only.
