...
2. `playwright-cli -s=tea-nfr network` → capture response times and payload
   sizes
...
> `playwright-cli -s=tea-nfr close`. Do NOT use `close-all` — it kills every
...
- **If `{outputFile}` already exists**, update:
  - Add `'step-03-gather-evidence'` to `stepsCompleted` array (only if not
    already present)
