...
Read outputs from 4 parallel NFR subagents, calculate overall risk level,
aggregate compliance status, and identify cross-domain risks.
...
- **If `{outputFile}` already exists**, update:
  - Add `'step-04e-aggregate-nfr'` to `stepsCompleted` array (only if not
    already present)
