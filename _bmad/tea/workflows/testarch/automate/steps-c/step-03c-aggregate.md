...
Read outputs from parallel subagents (API + E2E and/or Backend test generation
based on `{detected_stack}`), aggregate results, and create supporting
...
**Read E2E test subagent output (if {detected_stack} is `frontend` or
`fullstack`):**
...
**Read Backend test subagent output (if {detected_stack} is `backend` or
`fullstack`):**
...
- ✅ All test files written to disk (API + E2E and/or Backend, based on
  `{detected_stack}`)
...
- **If `{outputFile}` already exists**, update:
  - Add `'step-03c-aggregate'` to `stepsCompleted` array (only if not already
    present)
