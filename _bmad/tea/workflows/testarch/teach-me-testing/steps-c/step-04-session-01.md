...
description:
  'Session 1: Quick Start - TEA Lite intro, run automate workflow (30 min)'
...
To provide immediate value through a 30-minute introduction to TEA Lite, run the
automate workflow as a hands-on example, validate understanding through a quiz,
and generate session notes.

## MANDATORY EXECUTION RULES (READ FIRST):

### Universal Rules:

- 🛑 NEVER generate _unsolicited_ content without user input (session flow
  content is allowed once session begins)
...
unless user explicitly requests a change.
...
- **9 Workflows:** Teach Me Testing, Framework, Test Design, ATDD, Automate,
  Test Review, Trace, NFR Assessment, CI
- **35 Knowledge Fragments:** Distilled expertise on patterns, best practices,
  Playwright Utils
- **Quality Standards:** Definition of Done with execution limits (no flaky
  tests, no hard waits, etc.)
- **Risk-Based Testing:** P0-P3 matrix for prioritizing test coverage

**Why TEA exists:** Testing knowledge doesn't scale through manual teaching. TEA
makes testing expertise accessible through:

- Structured workflows that guide you step-by-step
- Documentation (32 docs) organized by type (tutorials, how-to, explanation,
  reference)
...
3. **TEA Integrated:** Full lifecycle - Framework → Test Design → ATDD/Automate
   → Review → Trace
...
{If role == QA:} **For QA Engineers:** This helps you quickly expand test
coverage without writing every test manually. Focus on test design, let TEA
handle boilerplate.

{If role == Dev:} **For Developers:** This generates tests following best
practices so you can focus on implementation. Tests are maintainable and follow
fixture patterns.

{If role == Lead:} **For Tech Leads:** This standardizes test architecture
across your team. Everyone writes tests the same way using TEA patterns.

{If role == VP:} **For VPs:** This scales testing across teams without manual
training. New hires can generate quality tests from day one.
...
**Note:** We won't actually run the workflow now (you can do that on your
project later), but you understand the concept."
...
**1. TEA is a framework:** Not just docs, but executable workflows that guide
you

**2. Risk-based testing:** Prioritize what matters (P0 critical, P3
nice-to-have)
...
**4. Engagement models:** Choose how much TEA you need (Lite → Solo → Integrated
...
Let me ask you 3 questions to validate your understanding. Passing score: ≥70%
(2 of 3 correct)."
...
A) Replace all testing tools with a single framework B) Make testing expertise
accessible through structured workflows and knowledge C) Automate 100% of test
writing D) Only works for Playwright tests
...
- If incorrect: "❌ Not quite. TEA's purpose is to make testing expertise
  accessible through structured workflows and knowledge (B). It's not about
  replacing tools or automating everything."
...
A) Prioritizing test coverage based on criticality B) Grading test code quality
C) Measuring test execution speed D) Tracking bug severity
...
- Correct answer: A
- If correct: "✅ Correct! P0-P3 helps prioritize what to test based on risk and
  criticality."
- If incorrect: "❌ The P0-P3 matrix is about prioritizing test coverage (A). P0
  = critical features like login, P3 = nice-to-have like tooltips."
...
- If incorrect: "❌ TEA Lite (B) is the quick start approach. Enterprise and
  Integrated are more comprehensive."
...
{If score >= 70:} ✅ **Passed!** You've demonstrated understanding of Session 1
concepts.
...
**Learner:** {user_name} ({role}) **Completed:** {current_date} **Score:**
{score}/100 **Duration:** 30 min
...
3. **Quality Standards:** Definition of Done (no flaky tests, no hard waits,
   stateless, self-cleaning)
...
- TEA Overview:
  https://bmad-code-org.github.io/bmad-method-test-architecture-enterprise/explanation/tea-overview/
- TEA Lite Quickstart:
  https://bmad-code-org.github.io/bmad-method-test-architecture-enterprise/tutorials/tea-lite-quickstart/
- Automate Workflow:
  https://bmad-code-org.github.io/bmad-method-test-architecture-enterprise/how-to/workflows/run-automate/
...
2. What does the P0-P3 risk matrix help with? → {user_answer}
   ({correct/incorrect})
3. Which TEA engagement model is best for quick value? → {user_answer}
   ({correct/incorrect})
...
{If experience_level == 'beginner':} **Session 2: Core Concepts** - Learn
testing fundamentals and TEA principles

{If experience_level == 'intermediate':} **Session 2 or 3** - Review concepts or
dive into architecture patterns

{If experience_level == 'experienced':} **Session 7: Advanced Patterns** -
Explore 35 knowledge fragments

---

**Generated by:** TEA Academy - Teach Me Testing Workflow **Session Path:**
Session 1 of 7
```
...
- If previous status for `session-01-quickstart` is not `completed`, increment
  `sessions_completed` by 1 (otherwise leave unchanged)
...
You've completed your first step in TEA Academy! You now understand what TEA is,
how TEA Lite works, and the different engagement models.

**Next:** You'll return to the session menu where you can choose Session 2 or
explore any other session.

**Progress:** {completion_percentage}% complete ({sessions_completed} of 7
sessions)"

### 10. Present MENU OPTIONS

Display: **Select an Option:** [A] Advanced Elicitation [P] Party Mode [C]
Continue to Session Menu
...
- IF C: Progress file already updated in step 8, then load, read entire file,
  then execute {nextStepFile}
- IF Any other: help user, then
...
This pattern repeats for all 7 sessions.
