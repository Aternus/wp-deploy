...
Select execution mode deterministically, then generate FAILING API and E2E tests
(TDD RED PHASE) with consistent output contracts across agent-team, subagent, or
...
- Available context: config, acceptance criteria from Step 1, test strategy from
  Step 3
...
**Master Rule:** TDD RED PHASE requires FAILING tests (with test.skip()). Mode
selection changes orchestration, never red-phase requirements.
