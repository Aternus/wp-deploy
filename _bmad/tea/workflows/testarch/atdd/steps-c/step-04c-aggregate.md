...
Read outputs from parallel subagents (API + E2E failing test generation),
aggregate results, verify TDD red phase compliance, and create supporting
infrastructure.
...
**Create fixtures needed by ATDD tests:** (Similar to automate workflow, but may
be simpler for ATDD since feature not implemented)
...
Feature endpoints to implement: {list endpoints from API tests}

UI components to implement: {list UI flows from E2E tests}
```
...
- **If `{outputFile}` already exists**, update:
  - Add `'step-04c-aggregate'` to `stepsCompleted` array (only if not already
    present)
...
**Master Rule:** TDD RED PHASE requires ALL tests to use test.skip() and assert
expected behavior.
