...
- Record heuristic signals:
  ...
  - Error-path coverage present/missing (validation, timeout, network/server
    failures)
...
- Auth/authz criteria include at least one denied/invalid-path test where
  applicable
...
- **If `{outputFile}` already exists**, update:
  - Add `'step-03-map-criteria'` to `stepsCompleted` array (only if not already
    present)
