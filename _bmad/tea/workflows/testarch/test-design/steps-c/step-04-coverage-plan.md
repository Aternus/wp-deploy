...
description:
  'Design test coverage, priorities, execution strategy, and estimates'
...
Create the test coverage matrix, prioritize scenarios, and define execution
strategy, resource estimates, and quality gates.
...
- **Nightly/Weekly**: Long-running or expensive suites (perf, chaos, large
  datasets)
...
- **If `{outputFile}` already exists**, update:
  - Add `'step-04-coverage-plan'` to `stepsCompleted` array (only if not already
    present)
