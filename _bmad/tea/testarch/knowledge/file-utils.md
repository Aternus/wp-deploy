...
Read and validate files (CSV, XLSX, PDF, ZIP) with automatic parsing, type-safe
results, and download handling. Simplify file operations in Playwright tests
with built-in format support and validation helpers.
...
- **Format support**: Multiple sheet support (XLSX), text extraction (PDF),
...
Text extraction may fail for PDFs that store text as vector graphics (e.g.,
those generated by jsPDF):
...
> **Note**: When `mergePages: false`, `content` is an array of strings (one per
> page). When `maxPages` is set, only that many pages are extracted.
...
> **Note**: When `fileToExtract` is specified, only that file appears in
> `extractedFiles`.
