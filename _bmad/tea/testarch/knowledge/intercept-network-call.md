...
Intercept network requests with a single declarative call that returns a
Promise. Automatically parse JSON responses, support both spy (observe) and stub
(mock) patterns, and use powerful glob pattern matching for URL filtering.
...
**Context**: Intercepting different endpoints in same test - setup order is
critical.
...
**Context**: Each `interceptNetworkCall` captures only the first matching
request.
...
The utility uses [picomatch](https://github.com/micromatch/picomatch) for
powerful glob pattern matching, dramatically simplifying URL targeting:
