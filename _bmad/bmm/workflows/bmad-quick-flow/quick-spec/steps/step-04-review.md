...
- IF A: Read fully and follow: `{advanced_elicitation}` with current spec
  content, process enhanced insights, ask user "Accept improvements? (y/n)", if
  yes update spec then redisplay menu, if no keep original then redisplay menu
- IF B: Read the entire workflow file at `{quick_dev_workflow}` and follow the
  instructions with the final spec file (warn: fresh context is better)
...
1. **Invoke Adversarial Review Task**:
   ...
   > With `{finalFile}` constructed, load and follow the review task. If
   > possible, use information asymmetry: load this task, and only it, in a
   > separate subagent or process with read access to the project, but no
   > context except the `{finalFile}`. <invoke-task>Review {finalFile} using
