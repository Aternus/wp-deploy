...
description:
  'Document Discovery & Confirmation - Handle fresh context validation, confirm
  PRD path, discover input documents'
...
Handle fresh context validation by confirming PRD path, discovering and loading
input documents from frontmatter, and initializing the validation report.
...
- Available context: PRD path (user-specified or discovered), workflow
  configuration
...
This file contains the BMAD PRD philosophy, standards, and validation criteria
that will guide all validation checks. Internalize this understanding - it
defines what makes a great BMAD PRD.
...
- "I couldn't find any PRD files in {planning_artifacts}. Please provide the
  path to the PRD file you want to validate."
...
- If not found: "I cannot find a PRD at that path. Please check the path and try
  again."
...
**If no inputDocuments array exists:** Note this and proceed with PRD-only
validation
...
**Are there any additional reference documents you'd like me to include in this
validation?**
...
**PRD Being Validated:** {prd_path} **Validation Date:** {current_date}
...
- User can ask questions or add more documents - always respond and redisplay
  menu

#### Menu Handling Logic:

- IF A: Read fully and follow: {advancedElicitationTask}, and when finished
  redisplay the menu
- IF P: Read fully and follow: {partyModeWorkflow}, and when finished redisplay
  the menu
- IF C: Read fully and follow: {nextStepFile} to begin format detection
- IF user provides additional document: Load it, update report, redisplay
  summary
...
**Master Rule:** Complete discovery and setup BEFORE validation. This step
ensures everything is in place for systematic validation checks.
